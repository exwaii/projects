...
for i in range(len(mlist)):
    for j in range(16):
        if mlist[i][j]*mlist[i][j+1]*mlist[i][j+2]*mlist[i][j+3]>maxthing:
            maxthing=mlist[i][j]*mlist[i][j+1]*mlist[i][j+2]*mlist[i][j+3]
        elif mlist[j][i]*mlist[j+1][i]*mlist[j+2][i]*mlist[j+3][i]>maxthing:
            maxthing=mlist[j][i]*mlist[j+1][i]*mlist[j+2][i]*mlist[j+3][i]
        #print(j)
        #print(i)
for i in range(16):
    for j in range(16):
        if mlist[i][j]*mlist[i+1][j+1]*mlist[i+2][j+2]*mlist[i+3][j+3]>maxthing:
            maxthing=mlist[i][j]*mlist[i+1][j+1]*mlist[i+2][j+2]*mlist[i+3][j+3]
for i in range(3,20):
    for j in range(16):
        if mlist[i][j]*mlist[i-1][j+1]*mlist[i-2][j+2]*mlist[i-3][j+3]>maxthing:
            maxthing=mlist[i][j]*mlist[i-1][j+1]*mlist[i-2][j+2]*mlist[i-3][j+3]
